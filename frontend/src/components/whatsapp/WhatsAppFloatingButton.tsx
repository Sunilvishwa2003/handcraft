  ...
  return (
    <>
      <motion.button
        ...
        whileTap={{ scale: 0.96 }}
        className="fixed bottom-5 left-4 z-[60] flex items-center gap-3 rounded-full bg-[#25D366] px-4 py-3 text-sm font-semibold text-slate-950 shadow-[0_22px_48px_rgba(37,211,102,0.32)] ring-1 ring-white/50 md:bottom-8 md:left-8"
      >
        ...
        <span className="relative hidden sm:block">Talk to Artisan</span>
      </motion.button>

      <WhatsAppInquiryModal open={open} onClose={() => setOpen(false)} title="Start a handcrafted conversation" />
    </>
  );
}
